...
STATIC void mv_pars_to(string target)
{
	/*
	 * Close the temporary file and either move it to the target,
	 * or remove it.  Revert its name to the template.
	 */
	static const char template[] = PARSERFILE;
	...
	if (target != NULL)
		...
		remove(f_pars);
	memcpy(f_pars, template, sizeof(template));
}
...
void install(string target, string source)
{
	 ...
	 * by LLgen from the source, or that the target is not
	 * present).  Remove the temporary file.
	 */
	register int c1, c2;
	register FILE *f1, *f2;
	int cnt;
	...
	/*
	 * Must have opened temporary at beginning of file
	 */
	f1 = fpars;
	/*
